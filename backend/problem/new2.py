...
def save_to_database(df, source='origin'):  # 修改默认source为origin
    """保存数据到数据库"""
    try:
        # 连接数据库
...
def crawl_and_save():
    """爬取数据并保存的主函数"""
    # 创建邮件日志对象
    mail_logger = Mail_log("中药材产地价格爬虫")  # 修改日志名称
    ...
    try:
        ...
        # 爬取数据
        for x in range(1, 37):  # 这里可以根据需要修改页数范围
            try:
                # 修改URL为产地价格页面
        ...
        if not data:
            ...
            mail_logger.mail_send("dingryl@foxmail.com", "中药材产地价格爬虫运行异常")
            return
        ...
        try:
            ...
            csv_path = os.path.join(data_dir, f'{current_date}产地价格.csv')
            ...
            mail_logger.mail_send("dingryl@foxmail.com", "中药材产地价格爬虫数据保存异常")
            return
